...
class AdminPostController extends Controller
{
    ...
    public function store(Request $request)
    {
       $request->validate([
        ...
        //Wysyłanie artykułu do subskrybentów
        if($request->subscriber_send_option == 1)
        {
            $subject = 'Nowy artykul zostal opublikowany';
            $message = 'Cześć, w naszej witrynie zostal opublikowany nowy artykul. Prosze wejdz na strone i zobacz artykul:<br>';
    ...
    {
        // nie pozwalam adminowi edytować artykułów innych autorów
    ...
    {
        // nie pozwalam adminowi usuwać artykułów innych autorów
        $test = Post::where('id',$id)->where('admin_id',Auth::guard('admin')->user()->id)->count();
        if(!$test) 
        {
            return redirect()->route('admin_home');
